...
# ─────────────────────────────────────────────────────────────────────────────
# 2b. Güncelleme paketi — update-engine.ps1'in tükettiği, versiyonlu .zip
# ─────────────────────────────────────────────────────────────────────────────
# Her sürümde aynı formatta tek bir paket üretir: app\ (yayın çıktısı) +
# version.txt + update-engine.ps1 (kendi kendine yeten, tek klasöre çıkarılıp
# doğrudan çalıştırılabilir). Kurulum EXE'sinden ayrı — mevcut müşterilerde
# SQL Express/IIS/Hosting Bundle'ı tekrar kurmaya gerek yoktur, sadece
# uygulama dosyaları + (varsa) yeni EF Core migration'ları taşınır.
function New-UpdatePackage {
    Write-Step 'Güncelleme paketi hazırlanıyor'

    # update-engine.ps1'in okuyacağı sürüm bilgisi — kurulu VERSION.txt ile
    # karşılaştırılır (bkz. installer/update-engine.ps1).
    Set-Content -Path (Join-Path $AppDir 'VERSION.txt') -Value $Version -Encoding UTF8

    $updDir = Join-Path $BuildDir 'update'
    if (Test-Path $updDir) { Remove-Item $updDir -Recurse -Force }
    New-Item -ItemType Directory -Path $updDir -Force | Out-Null

    Copy-Item -Path $AppDir -Destination (Join-Path $updDir 'app') -Recurse -Force
    Set-Content -Path (Join-Path $updDir 'version.txt') -Value $Version -Encoding UTF8
    Copy-Item -Path (Join-Path $PSScriptRoot 'update-engine.ps1') -Destination $updDir -Force

    if (-not (Test-Path $DistDir)) { New-Item -ItemType Directory -Path $DistDir -Force | Out-Null }
    $zipPath = Join-Path $DistDir "FscErpUpdate-$Version.zip"
    if (Test-Path $zipPath) { Remove-Item $zipPath -Force }
    Compress-Archive -Path (Join-Path $updDir '*') -DestinationPath $zipPath -Force

    $hash = (Get-FileHash $zipPath -Algorithm SHA256).Hash
    $size = (Get-Item $zipPath).Length / 1MB
    Write-Ok ("Güncelleme paketi: $zipPath ({0:N1} MB, SHA256 {1})" -f $size, $hash)
    return $zipPath
}

# ─────────────────────────────────────────────────────────────────────────────
# 2c. GitHub Release yayınla — internet erişimi olan müşterilerin uygulama
#     içinden indirebilmesi için. Repo (goktaso/FSCTakip-Releases) SADECE bu
#     .zip paketlerini barındırır — kaynak kod hiç girmez, PRIVATE tutulur.
#     Müşteri tarafı, appsettings.json'daki tek-repo'ya salt-okunur ince taneli
#     (fine-grained) bir token ile indirir (bkz. docs/UPDATE_DAGITIMI.md).
# ─────────────────────────────────────────────────────────────────────────────
function Publish-GitHubRelease {
    param([string] $ZipPath)

    Write-Step 'GitHub Release yayınlanıyor (goktaso/FSCTakip-Releases)'

    $gh = Get-Command gh -ErrorAction SilentlyContinue
    if (-not $gh) {
        throw "GitHub CLI (gh) bulunamadı. -PublishRelease olmadan derlemeyi tekrarlayın veya gh CLI kurun."
    }

    $tag = "v$Version"
    & gh release create $tag $ZipPath `
        --repo 'goktaso/FSCTakip-Releases' `
        --title "FSC Takip ERP $Version" `
        --notes "Otomatik yayınlandı: $(Get-Date -Format 'dd.MM.yyyy HH:mm')"
    if ($LASTEXITCODE -ne 0) { throw "GitHub Release oluşturulamadı (çıkış kodu $LASTEXITCODE)." }

    Write-Ok "Yayınlandı: https://github.com/goktaso/FSCTakip-Releases/releases/tag/$tag"
}
...
Test-NoSecretLeak
$updateZip = New-UpdatePackage

if ($UpdateOnly) {
    # Mevcut kurulumlara sadece güncelleme paketi göndermek için — SQL/IIS/Hosting
    # Bundle zaten kurulu, tam kurulum EXE'sini (yavaş: prereq indirme + Inno Setup)
    # yeniden derlemeye gerek yok.
    Write-Host ''
    Write-Host '════════════════════════════════════════════════════' -ForegroundColor Green
    Write-Host ' GÜNCELLEME PAKETİ HAZIR (-UpdateOnly)' -ForegroundColor Green
    Write-Host '════════════════════════════════════════════════════' -ForegroundColor Green
    Write-Host "  Paket: $updateZip"
    Write-Host ''
    if ($PublishRelease) {
        Publish-GitHubRelease -ZipPath $updateZip
    } else {
        Write-Warn "GitHub Release yayınlanmadı (-PublishRelease vermediniz)."
    }
    return
}
...
Write-Host '════════════════════════════════════════════════════' -ForegroundColor Green
Write-Host "  Kurulum EXE : $exe"
Write-Host ("  Boyut       : {0:N1} MB" -f $size)
Write-Host "  SHA256      : $hash"
Write-Host "  Güncelleme  : $updateZip  (mevcut müşterilerde SQL/IIS'i tekrar kurmadan sadece dosya+migration günceller)"
Write-Host ''

if ($PublishRelease) {
    Publish-GitHubRelease -ZipPath $updateZip
} else {
    Write-Warn "GitHub Release yayınlanmadı (-PublishRelease vermediniz). İnternetli müşteriler bu sürümü uygulama-içi 'Güncelleme Kontrol Et' ile göremez."
}
Write-Host ''
